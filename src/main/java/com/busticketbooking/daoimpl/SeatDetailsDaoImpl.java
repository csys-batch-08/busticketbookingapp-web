...
public class SeatDetailsDaoImpl {
	...
	public boolean ticketexist(int ticketCountEntered, String ticketRandomNumber, Bus bus, User user) {
		...
		try {
			con = ConnectionUtill.connectdb();
			String query = "select ticket_no,user_id,bus_id,seat_no,seat_status from seat_details where seat_no=? "
					+ " and bus_id=? and seat_status in 'booked'";
	...
	public List<SeatDetails> showSeatList() {
		String seatDetailsQuery = "select ticket_no,user_id,bus_id,seat_no,seat_status from seat_details "
		...
		try {
			...
			while (rs.next()) {
				...
				SeatDetails seatDetails = new SeatDetails(bookTickets, rs.getInt("seat_no"),
