...
public class OperatorDaoImpl implements OperatorDAO {
	...
	public boolean insertOperator(Operator OperatorModel) {
		String insertbus = "insert into bus_operators (OPERATOR_NAME,OPERATOR_EMAIL,OPERATOR_CONTACT, OPERATOR_AGE)"
				+ " values (?,?,?,?)";
		int result = 0;
		Connection con = null;
	...
	public boolean updateOperator(Operator operatorModel) {
		...
		String operatorUpdate = "update bus_operators set operator_name=?, operator_email=?, operator_contact=?, "
				+ "operator_age=? where operator_id=?";
	...
	public List<Operator> viewOperator() {
		...
		String operatorView = "select operator_id,operator_name,operator_email,operator_contact,operator_age,"
				+ "operator_status from bus_operators";
		...
		try {
			...
			while (rs.next()) {
				Operator operator = new Operator(rs.getInt("operator_id"), rs.getString("operator_name"),
						rs.getString("operator_email"), rs.getLong("operator_contact"), rs.getInt("operator_age"),
						rs.getString("operator_status"));
	...
	public Operator getOperatorById(int operatorId) {
		...
		String getOperator = "select operator_id,operator_name,operator_email,operator_contact,operator_age,"
				+ "operator_status from bus_operators where operator_id=?";
		Connection con = null;
		PreparedStatement pstatement = null;
		...
		try {
			...
			rs = pstatement.executeQuery();
			if (rs.next()) {
				operatormodel = new Operator(rs.getInt("operator_id"), rs.getString("operator_name"),
						rs.getString("operator_email"), rs.getLong("operator_contact"), rs.getInt("operator_age"),
						rs.getString("operator_status"));
			}
		} catch (SQLException e) {
