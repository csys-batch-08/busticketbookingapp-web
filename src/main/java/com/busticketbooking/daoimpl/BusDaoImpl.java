...
public class BusDaoImpl implements BusDAO {
	...
	public boolean insertBus(Bus busModel) {
		String busInsert = "insert into bus_details (bus_category, from_city, to_city, departure,"
				+ " arrival, seater_fare, total_seat,seat_status) values (?,?,?,?,?,?,?,?)";
		int result = 0;
		Connection con = null;
	...
	public boolean updateBus(Bus busModel) {
		...
		String busUpdate = "update bus_details set bus_category=?, from_city=?, to_city=?, departure=?,"
				+ " arrival=?, seater_fare=?, total_seat=?,seat_status=? where bus_id=?";
		int result = 0;
		Connection con = null;
		PreparedStatement pstatement = null;
		try {
	...
	public List<Bus> viewAllBus() {
		...
		String busView = "select bus_id,bus_no,operator_id,bus_category,from_city,to_city,departure,"
				+ "arrival,seater_fare,total_seat,seat_status from bus_details";
		...
		try {
			...
			while (rs.next()) {
				busModel = new Bus(rs.getInt("bus_Id"), rs.getInt("bus_No"), rs.getInt("operator_Id"),
						rs.getString("bus_Category"), rs.getString("from_City"), rs.getString("to_City"),
						rs.getTimestamp("Departure").toLocalDateTime(), rs.getTimestamp("Arrival").toLocalDateTime(),
						rs.getInt("seater_Fare"), rs.getInt("total_Seat"), rs.getString("seat_Status"));
	...
	public List<Bus> searchhBus(LocalDate givenDepartureDate, String fromLocation, String toLocation) {
		String findBus = "select bus_id,bus_no,operator_id,bus_category,from_city,to_city,departure,arrival,"
				+ "seater_fare,total_seat,seat_status from bus_details where to_char(departure,'dd-mm-yyyy')=?"
				+ " and from_city=? and to_city=? order by departure";
		Connection con = null;
		PreparedStatement pstatement = null;
		Bus busModel;
		ResultSet rs = null;
	...
	public Bus findBusDetailsUsingID(int busId) {
		String getBus = "select bus_id,bus_no,operator_id,bus_category,from_city,to_city,departure,"
				+ "arrival,seater_fare,total_seat,seat_status from bus_details where bus_id=?";
		Connection con = null;
		PreparedStatement pstatement = null;
		Bus busModel = null;
		ResultSet rs = null;
