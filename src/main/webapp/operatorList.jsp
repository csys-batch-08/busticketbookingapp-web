...
<body>
	...
	<c:choose>
		...
		<c:when test="${delete!=null}">
			<script>
		popupMessages('Deleted successfully')
	</script>
		</c:when>
					...
					<tr>
						...
						<c:if test="${operator.getOperatorStatus().equals('active') }">
							<td><button id="editbutton"
									onclick="editOperator(${operator.getOperatorId()})">Edit</button>
								/
								<button id="deletebutton"
									onclick="deleteOperator(${operator.getOperatorId()})">Delete</button></td>
						</c:if>
